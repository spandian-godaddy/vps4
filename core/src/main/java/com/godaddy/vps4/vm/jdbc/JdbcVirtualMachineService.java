...
public class JdbcVirtualMachineService implements VirtualMachineService {
    ...
    public VirtualMachineSpec getSpec(String name) {
        ...
        return Sql.with(dataSource).exec("SELECT spec_id, spec_name, tier, cpu_core_count, " +
                "memory_mib, disk_gib, valid_on as \"spec_valid_on\", " +
                "valid_until as \"spec_valid_until\" FROM virtual_machine_spec " +
                " WHERE spec_name=? ",
    ...
    public List<VirtualMachine> listVirtualMachines(long projectId) {
        return Sql.with(dataSource).exec("SELECT vm.vm_id, vm.orion_guid, vm.project_id, vm.name as \"vm_name\", " +
                ...
                "vms.valid_until as \"spec_valid_until\", image.name as \"image_name\" " +
    ...
    protected VirtualMachineSpec mapVirtualMachineSpec(ResultSet rs) throws SQLException {
        ...
        return new VirtualMachineSpec(rs.getInt("spec_id"),
                ...
                rs.getInt("disk_gib"),
                rs.getTimestamp("spec_valid_on").toInstant(),
                validUntil != null ? validUntil.toInstant() : null );
    }
...
//                vmId);
    ...
    public void createVirtualMachineRequest(UUID orionGuid, String operatingSystem, String controlPanel, int tier, int managedLevel) { 
        Sql.with(dataSource).exec("SELECT * FROM virtual_machine_request_create(?,?,?,?,?)",
                null,
                orionGuid,
                operatingSystem,
                tier,
    ...
    private VirtualMachineRequest mapVirtualMachineRequest(ResultSet rs) throws SQLException {
        ...
        return new VirtualMachineRequest(java.util.UUID.fromString(rs.getString("orion_guid")),
                rs.getInt("tier"),
                rs.getInt("managed_level"),
                rs.getString("operating_system"),
                rs.getString("control_panel"),
                rs.getTimestamp("create_date").toInstant(),
                provisionDate != null ? provisionDate.toInstant() : null);
    }
    ...
    public void provisionVirtualMachine(long vmId, UUID orionGuid, String name, long projectId, int specId, int managedLevel, int imageId) {
        Sql.with(dataSource).exec("SELECT * FROM virtual_machine_provision(?, ?, ?, ?, ?, ?, ?)", null,
                vmId, orionGuid, name, projectId, specId, managedLevel, imageId);
	}
}
