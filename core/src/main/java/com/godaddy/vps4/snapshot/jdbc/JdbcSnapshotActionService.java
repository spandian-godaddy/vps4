...
public class JdbcSnapshotActionService implements ActionService {
    ...
    @Override
    public long createAction(UUID snapshotId, ActionType actionType, String request, long userId) {
        return Sql.with(dataSource).exec("INSERT INTO snapshot_action (snapshot_id, action_type_id, request, vps4_user_id) "
                + "VALUES (?, ?, ?::json, ?) RETURNING id;",
                Sql.nextOrNull(rs -> rs.getLong("id")), snapshotId, actionType.getActionTypeId(), request, userId);
    }
