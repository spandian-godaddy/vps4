...
public class EndRescueVmTest {
    ...
    EndRescueVm command = new EndRescueVm(vmService);

    @Test
    public void testExecute() {
        VmAction hfsRescueAction = mock(VmAction.class);
        when(vmService.endRescueVm(hfsVmId)).thenReturn(hfsRescueAction);

        assertNull(command.execute(context, hfsVmId));
        verify(vmService).endRescueVm(hfsVmId);
