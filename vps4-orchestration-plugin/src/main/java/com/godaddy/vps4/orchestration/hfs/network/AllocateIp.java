...
public class AllocateIp implements Command<AllocateIp.Request, IpAddress> {
    ...
    public IpAddress execute(CommandContext context, AllocateIp.Request request) {
        ...
        IpAddress hfsAddress = networkService.getAddress(hfsAction.addressId);
        IpAddressValidator.validateIpAddress(hfsAddress.address);
        ...
        if (request.serverId == null && hfsAddress.status != Status.UNBOUND) {
            throw new RuntimeException(String.format("IP %s is not unbound", hfsAddress.address));
        }

        logger.info("Address allocate is complete: {}", hfsAction);
        logger.info("Allocated address: {}", hfsAddress);
