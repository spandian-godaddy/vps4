...
public class DestroyVmTest {
    ...
    public void testDestroyVm() throws VmNotFoundException {
        ...
        UUID orionGuid = orionGuids.get(0);

        Instant beforeValidUntil = virtualMachineService.getVirtualMachine(vmId).validUntil;

        Action action = newValidVmResource().destroyVm(vmId);
        Assert.assertNotNull(action.commandId);
        verify(creditService, times(1)).unclaimVirtualMachineCredit(orionGuid);

        Instant afterValidUntil = (virtualMachineService.getVirtualMachine(vmId).validUntil);

        //Valid until starts as infinity, but when it's deleted, it changes to now().
        Assert.assertFalse(beforeValidUntil.equals(afterValidUntil));
