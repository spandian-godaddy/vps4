...
public class SsoTokenExtractor {
    ...
    private final SsoTokenReader tokenReader = new SsoTokenReader();

    private final long sessionTimeoutMs;

    public SsoTokenExtractor(KeyService keyService) {
        this(keyService, DEFAULT_SESSION_TIMEOUT_MS);
    }

    public SsoTokenExtractor(KeyService keyService, long sessionTimeoutMs) {
    ...
    private final SsoTokenReader ssoTokenReader = new SsoTokenReader();
    ...
    public SsoToken extractAuthorizationHeaderToken(HttpServletRequest request) {
        ...
        if (authHeader != null) {
            ...
            if (authHeader.startsWith(AUTH_HEADER_PREFIX)) {
                ...
                try {
                    SsoToken ssoToken = ssoTokenReader.readSsoToken(token);
                    if (ssoToken != null) {
                        try {
                            validate(ssoToken);
    ...
    protected boolean verify(SignedJWT jwt) throws VerificationException {
        ...
        JWK key = keyService.getKey(keyId);
        if (key == null) {
